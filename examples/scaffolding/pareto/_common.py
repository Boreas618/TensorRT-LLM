...
def atomic_write_json(path: Path, data: Any) -> None:
    ...
    fd, tmp_local = tempfile.mkstemp(
        suffix=".json", prefix=path.stem + ".", dir="/tmp"
    )
    try:
        ...
        try:
            subprocess.run(["cp", tmp_local, lustre_tmp],
            ...
            raise RuntimeError(
                f"Lustre cp failed for {path}; data dumped to stdout"
            ) from None

        try:
            subprocess.run(["mv", lustre_tmp, str(path)],
