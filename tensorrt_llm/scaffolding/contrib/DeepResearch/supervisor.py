...
from .prompts import (final_report_generation_prompt,
                      generate_research_brief_prompt, supervisor_system_prompt)
...
class Supervisor(Controller):
    ...
    def process(self, tasks: List[Task], **kwargs):
        ...
        # can be added.
        user_messages = [
            UserMessage(content=supervisor_task.input_str).to_dict()
        ]
        ...
        research_brief_messages = [
            UserMessage(
                generate_research_brief_prompt.format(
                    date=get_today_str(), messages=user_messages)).to_dict()
        ]
        ...
        supervisor_prompt_messages = [
            SystemMessage(
                supervisor_system_prompt.format(
                    ...
                    max_concurrent_research_units=self.
        ...
        for _ in range(self.max_research_iter):
            ...
            if len(research_tasks) != 0:
                ...
                for task in research_tasks:
                    supervisor_tools_messages.append(
                        UserMessage(task.research_result).to_dict())
        ...
        final_report_generation_task = ChatTask.from_messages(messages=[
            SystemMessage(
                final_report_generation_prompt.format(
                    research_brief=research_brief,
                    messages=supervisor_prompt_messages +
                    supervisor_tools_messages,
