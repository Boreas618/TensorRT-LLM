...
class KVCacheControlPlane:
    ...
    async def _truncate_kv_cache_tokens(
        self, request: KVCacheTruncateTokensRequest
    ) -> Response:
        """Handle :class:`KVCacheTruncateTokensRequest` — the token-level
        truncate variant used by trace replay.
        ...
        try:
            ...
            if len(prefixes) != len(keeps):
                return self._create_error_response(
                    ...
                    f"len(num_tokens_to_keep)={len(keeps)}",
                    HTTPStatus.BAD_REQUEST,
                )
            for i, (prefix, keep) in enumerate(zip(prefixes, keeps)):
                if keep < 0 or keep > len(prefix):
                    return self._create_error_response(
                        f"prefixes[{i}]: num_tokens_to_keep={keep} out of "
                        f"range [0, {len(prefix)}]",
                        HTTPStatus.BAD_REQUEST,
                    )
