...
class PyExecutor:
    ...
    def __init__(
        ...
        # ------------------------------------------------------------------
        self.iter_state_sample_path = (
            getattr(self.llm_args, "iter_state_sample_path", None) or None)
        self.iter_state_sample_every_iters = int(
            getattr(self.llm_args, "iter_state_sample_every_iters", 100)
            or 0)
        ...
        if (self.iter_state_sample_path
            ...
            try:
                self._iter_state_sample_file = open(
                    self.iter_state_sample_path, "w")
                logger.info(
    ...
    def _process_iter_stats(
        ...
        if self._last_iter_forward_start_cpu > 0.0 and self._last_iter_forward_end_cpu > 0.0:
            self._last_iter_schedule_ms = (
                self._last_iter_forward_start_cpu - batch_state.iter_start_time) * 1e3
            self._last_iter_forward_cpu_ms = (
                self._last_iter_forward_end_cpu - self._last_iter_forward_start_cpu) * 1e3
            self._last_iter_post_ms = (
                iter_end_time - self._last_iter_forward_end_cpu) * 1e3
            ...
            if self._last_iter_post_schedule_cpu > 0.0:
                self._last_iter_schedule_main_ms = (
                    self._last_iter_post_schedule_cpu - batch_state.iter_start_time) * 1e3
                self._last_iter_schedule_post_main_ms = (
                    self._last_iter_forward_start_cpu - self._last_iter_post_schedule_cpu) * 1e3
            else:
                self._last_iter_schedule_main_ms = 0.0
                self._last_iter_schedule_post_main_ms = 0.0
        else:
        ...
        stats = self._update_iter_stats(batch_state.iter_stats,
                                        iter_latency_ms,
    ...
    def _maybe_log_kv_monitor(self, stats) -> None:
        ...
        logger.info(
            f"{tag}[kvmon iter={getattr(stats, 'iter', '?')}] "
    ...
    def _maybe_sample_iter_state(self, stats) -> None:
        ...
        self._iter_state_buffer.append({
            ...
            "active": int(getattr(stats, "num_active_requests", 0) or 0),
            "max_active":
            int(getattr(stats, "max_num_active_requests", 0) or 0),
            "queued": int(getattr(stats, "num_queued_requests", 0) or 0),
            "ctx": int(getattr(ib, "num_context_requests", 0) or 0) if ib
            else 0,
            "gen":
            ...
            "iter_latency_ms": getattr(stats, "iter_latency_ms", None),
            ...
            "gpu_forward_ms": float(self._last_iter_gpu_forward_ms),
            ...
            "schedule_ms": float(self._last_iter_schedule_ms),
            ...
            "schedule_main_ms": float(self._last_iter_schedule_main_ms),
            "schedule_post_main_ms": float(self._last_iter_schedule_post_main_ms),
            "forward_cpu_ms": float(self._last_iter_forward_cpu_ms),
            "post_ms": float(self._last_iter_post_ms),
            ...
            "decode_deferred": bool(self._last_iter_decode_deferred),
            "decode_defer_streak":
    ...
    def _executor_loop_cleanup(self):
        ...
        if self._iter_state_sample_file is not None:
            try:
                ...
                logger.warning(
                    f"[iter_state_sampler] close failed: {e!r}")
            self._iter_state_sample_file = None
    ...
    def _decode_defer_requests(self, context_requests: list[LlmRequest],
        ...
        within_starvation_cap = (
            self.consecutive_decode_defer_iters < self.max_decode_defer_iters)
    ...
    def _forward_step(
        ...
        try:
            ...
            try:
                if self._fwd_timing_idx > 0 and prev_start.query() and prev_end.query():
                    self._last_iter_gpu_forward_ms = float(
                        prev_start.elapsed_time(prev_end))
            except Exception:
                pass
            cur_start, cur_end = self._fwd_timing_events[self._fwd_timing_idx % 2]
